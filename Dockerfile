...
# 避免国内镜像污染，强制仅用官方 CUDA whl，严格按 PyTorch 官网推荐组合
RUN pip install --retries 10 --timeout 600 --no-cache-dir torch==2.8.0 torchvision==0.19.0 torchaudio==2.8.0 --index-url https://download.pytorch.org/whl/cu128/torch_stable.html

# 拉取 ComfyUI 代码
RUN git clone --branch v0.3.51 https://github.com/comfyanonymous/ComfyUI.git /app
